...
//Code Here
let postTaxPrices = prices.map(function(elem){return elem * 1.07});
...
//Code Here
let totalPopulation = populations.reduce(function(tot, elem){return tot += elem}, tot = 0)
...
*/
let total = 0
let orderTotals = orders.map(function(bump) {
  total = ((bump.tax+1)*bump.price)
  return total
})
...
const bobsTotal = purchases
  ...
  .reduce((tot, arr) => tot + arr.price, 0)
